    ...
    def displayPieces(self):
        ...
        while True:
            ...
            if update:
                ...
                ax.set_xlim3d([0, np.size(piece_matrix, 2)])
                ax.set_ylim3d([0, np.size(piece_matrix, 1)])
                ax.set_zlim3d([0, np.size(piece_matrix, 0)])
                ax.set_box_aspect((np.size(piece_matrix, 2), np.size(piece_matrix, 1), np.size(piece_matrix, 0)))

                ax.set_xticks(list(range(0, np.size(piece_matrix, 2) + 1)))
                ax.set_yticks(list(range(0, np.size(piece_matrix, 1) + 1)))
                ax.set_zticks(list(range(0, np.size(piece_matrix, 0) + 1)))
